...
export default function EditListingPage({ params }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      ...
      showModal('Error', 'Could not save changes: ' + error.message, 'error')
    } else {
      showModal('Success!', 'Your listing has been updated successfully.', 'success', () => router.push('/dashboard'))
    }
  }
    ...
    setModal({
      ...
      confirmButton: {
        ...
        onClick: async () => {
          ...
          if (error) {
            showModal('Error', 'Could not unclaim listing: ' + error.message, 'error')
          } else {
            showModal('Unclaimed', 'You have successfully unclaimed this business.', 'success', () => router.push('/dashboard'))
          }
        }
      },
  ...
  const inputClass = "w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:border-[#007A5E] focus:outline-none"
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <p className="text-lg text-gray-600 mb-8">Update your business information</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="border-t-2 border-gray-200 pt-6">
            ...
            {imagePreview && (
              <div className="mb-4">
                ...
                {imageFile && (
                  <button
                    type="button"
                    onClick={() => { setImagePreview(formData.image_url || null); setImageFile(null) }}
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 mb-3">Upload a high-quality image of your business (max 5MB, JPG/PNG)</p>
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-[#007A5E] transition">
                <input type="file" accept="image/*" onChange={handleImageChange} className="hidden" id="image-upload" />
                <label htmlFor="image-upload" className="cursor-pointer">
                  <div className="text-4xl mb-2">📸</div>
                  <p className="text-gray-600 mb-1">{imagePreview ? 'Click to replace image' : 'Click to upload an image'}</p>
          ...
          <div>
            ...
            <input type="text" name="short_description" value={formData.short_description} onChange={handleChange} required maxLength="150" className={inputClass} />
          ...
          <div>
            ...
            <textarea name="description" value={formData.description} onChange={handleChange} required rows="6" className={inputClass + " resize-none"} />
          </div>
          ...
          <div className="border-t-2 border-gray-200 pt-6">
            ...
            <p className="text-sm text-gray-600 mb-4">Search for your address or click on the map to mark your exact location</p>
            ...
            <div className="mb-4">
              <div ref={mapRef} className="w-full h-96 rounded-lg border-2 border-gray-200 bg-gray-100" />
              {!mapLoaded && (
              ...
              <div className="bg-green-50 border-l-4 border-green-500 p-4 mb-4">
                <div className="flex items-start">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-green-800 mb-1">Location Set Successfully</p>
                    <p className="text-sm text-green-700 mb-2">{formData.address || 'Location marked on map'}</p>
                    <a href={`https://www.google.com/maps?q=${formData.latitude},${formData.longitude}`} target="_blank" rel="noopener noreferrer"
                      className="text-sm text-[#007A5E] hover:text-[#005F48] font-semibold">
            ...
            <div className="mt-4">
              <button type="button" onClick={() => setShowAdvancedLocation(!showAdvancedLocation)}
              ...
              {showAdvancedLocation && (
                <div className="mt-4 p-4 bg-gray-50 border border-gray-200 rounded-lg">
                  <p className="text-sm text-gray-600 mb-3">For advanced users: manually enter coordinates</p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-bold text-gray-900 mb-2">Latitude</label>
                      <input type="number" step="any" name="latitude" value={formData.latitude} onChange={handleChange} className={inputClass} placeholder="12.1165" />
                    </div>
                    <div>
                      <label className="block text-sm font-bold text-gray-900 mb-2">Longitude</label>
                      <input type="number" step="any" name="longitude" value={formData.longitude} onChange={handleChange} className={inputClass} placeholder="-61.6789" />
                    </div>
                  </div>
                </div>
              )}
          ...
          <div className="border-t-2 border-gray-200 pt-6">
            <div className="flex gap-4 mb-4">
              <Link href="/dashboard" className="flex-1 text-center bg-gray-200 text-gray-700 px-8 py-4 rounded-lg font-bold text-lg hover:bg-gray-300 transition">
                Cancel
              </Link>
              <button type="submit" disabled={saving || uploadingImage}
                className="flex-1 bg-[#007A5E] text-white px-8 py-4 rounded-lg font-bold text-lg hover:bg-[#005F48] transition disabled:opacity-50 disabled:cursor-not-allowed">
                {saving ? 'Saving...' : uploadingImage ? 'Uploading Image...' : 'Save Changes'}
              </button>
            </div>
            <button type="button" onClick={handleUnclaim}
